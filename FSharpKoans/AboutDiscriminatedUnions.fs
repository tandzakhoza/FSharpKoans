...
module ``07: The Good Kind of Discrimination`` = 
    ...
    let ``04 Pattern-matching using named fields`` () =
        let result =
            ...
            | v -> "correct" // <-- USE a pattern-match with named fields!
            | _ -> "nope"
    ...
    let ``06 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a = Node ("strin", Node ("x",Node("xx",Node("xxx",Empty,Empty),Empty),Empty),Empty) // <-- you may want to spread this over multiple lines and/or let-bindings ...!
