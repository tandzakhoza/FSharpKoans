    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            let rec count items acc =
                match items with 
                |[]-> acc
                |h::rest -> count rest (acc+1) 
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            let rec reverse list revlist=
                match  list with
                |[]-> revlist
                |h::rest -> reverse rest (h::revlist)
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec mapper list newlist =
                match list with
                |[]-> newlist
                |h::rest -> mapper rest (h+1::newlist)
            List.rev (mapper xs []) // write a function which adds 1 to each element
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec twice list newlist =
                match list with
                |[]-> newlist
                |h::rest -> twice rest (h*2::newlist)
            List.rev (twice xs []) // write a function which doubles each element
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec mapping (list: 'a list) newlist =
                match list with
                |[]-> newlist
                |h::rest -> mapping rest (f h::newlist)
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec find (list : 'a list) newlist=
                match list with 
                |[]-> newlist
                |h::rest -> match f h with 
                            |true -> find rest (h::newlist)
                            |false-> find rest newlist
            List.rev (find xs []) // write a function which filters based on the specified criteria
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
            let rec find (list : int list) newlist=
                match list with
                |[]-> newlist
                |h::rest -> match h%2=0 with
                            |true-> find rest newlist
                            |_-> find rest (h::newlist)
            List.rev (find xs []) // write a function to filter for odd elements only.
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            let rec count list sum =
                match list with
                |[] -> sum
                |h::rest -> count rest (sum+h)  
            count xs initialState // write a function to do what's described above
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
            let rec count list acc =
                match list with
                |[] -> acc
                |h::rest -> count rest (acc*h)  
            count xs initialState // write a function to multiply the elements of a list
    ...
    let ``16 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec count list acc=
                match list with 
                |[]-> acc
                |h::rest-> count rest (f acc h)
            count xs initialState  // write a function to do a fold.
        fold (+) 0 [1;2;3;4] |> should equal 10
    ...
    let ``18 exists: finding whether any matching item exists`` () =
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
            let rec isbool list result =
             match list with
             |[]-> result
             |h::rest -> match f h with
                         |false -> isbool rest result
                         |_-> true 
            isbool xs false// Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
    ...
    let ``19 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec partA list blist =
                match list with
                |[]-> blist
                |h::rest -> match f h with
                            |false ->  partA rest (h::blist)
                            |_-> partA rest (blist)
             // Does this: https://msdn.microsoft.com/en-us/library/ee353782.aspx
            let rec partB list alist =
                match list with
                |[]-> alist
                |h::rest -> match f h with
                            |false -> partB rest (alist)
                            |_-> partB rest (h::alist)
            List.rev (partB xs []), List.rev (partA xs [])
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec makelist num newlist =
                match num<0 with 
                |true-> newlist
                |_ -> makelist (num-1) (f num::newlist)
            makelist (n-1) []       // Does this: https://msdn.microsoft.com/en-us/library/ee370497.aspx
        ...
        let tryPick (p : 'a -> 'b option) (xs : 'a list) : 'b option =
            let rec check list =
                ...
                |h::rest -> match p h with 
                            |None -> check rest
                            |Some a -> Some a
            check xs // Does this: https://msdn.microsoft.com/en-us/library/ee353814.aspx
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    *)
    ...
    let ``23 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
            let rec makelist list newlist=
                match list with 
                |[]-> newlist
                |h::rest -> match p h with 
                            |None -> makelist rest newlist
                            |Some a -> makelist rest (a::newlist)// Does this: https://msdn.microsoft.com/en-us/library/ee353456.aspx
            List.rev (makelist xs []) 
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    let ``24 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            ...
            let rec makelist list num newlist=
                match list,num<length with 
                |[], false-> newlist  
                |h::rest, _ -> makelist rest (num+1) (f num h::newlist)
            List.rev (makelist xs 0 []) // Does this: https://msdn.microsoft.com/en-us/library/ee353425.aspx
